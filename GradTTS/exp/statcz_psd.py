...
def statcz_psd_mcd(prosody_dict, base_dir):
    ...
    For parallel test
    Args:
        prosody_dict:  {"emo1": {"model1": {"psd/phone/speechid": [[], []]}}}

    Returns:
        psd_mcd_stat_res: {"emo1": {"model1": [[p_diff], [e_diff], [mcd_res]]}}}
    """
    psd_mcd_stat_res = {}
    ...
    for emo, model_psd in prosody_dict.items():
        if emo not in psd_mcd_stat_res.keys():
            psd_mcd_stat_res[emo] = dict()

        for model_n, psd_phone_sid in model_psd.items():
            if model_n != "reference":
                if model_n not in psd_mcd_stat_res[emo].keys():
                    psd_mcd_stat_res[emo][model_n] = []
                #psd_len = len(psd)
                ...
                p_diffs = [diff_func(interpolate_nan(psd_phone_sid["pitch"][i]), interpolate_nan(prosody_dict[emo]["reference"]["pitch"][i]))
                          for i in range(wav_num)]
                e_diffs = [diff_func(interpolate_nan(psd_phone_sid["energy"][i]), interpolate_nan(prosody_dict[emo]["reference"]["energy"][i]))
                          for i in range(wav_num)]

                p_diffs_mean = mean_func(p_diffs)
                e_diffs_mean = mean_func(e_diffs)
                psd_mcd_stat_res[emo][model_n].append(p_diffs_mean)
                psd_mcd_stat_res[emo][model_n].append(e_diffs_mean)
                ...
                mcds = [
                    mcd_toolbox.calculate_mcd(wav_dir + "/" + psd_phone_sid["speechid"][i] + ".wav",
                                              ref_dir + "/" + prosody_dict[emo]["reference"]["speechid"][i]  + ".wav")
                    for i in range(wav_num)]

                mcd_res_mean = mean_func(mcds)
                psd_mcd_stat_res[emo][model_n].append(mcd_res_mean)
