...
from GradTTS.const_param import emo_melstyleSpk_dict, config_dir, logs_dir, melstyle_dir, wav_dir, wav_dict, \
    emo_num_dict
...
def get_styles_wavs_from_dict(style_dict):
    ...
    for emo, mel_spk in style_dict.items():
        ...
        emo_tensor = torch.LongTensor(get_emo_label(emo, emo_num_dict)).cuda()
...
def evaluate_prosody_similarity(
    ...
    if start_step <= 1 <= end_step:
        ...
        with torch.no_grad():
            for model_n, configs, chk_pt in ([model_name1, configs1, chk_pt1], [model_name2, configs2, chk_pt1]):
                # skip gradtts (temp)
                # if model_n == "gradtts":
                #    continue
                ...
                for i, style in enumerate(styles):  # Emo/mels change, spk same
                    ...
                    for j, text in enumerate(synTexts):
                        x = torch.LongTensor(intersperse(text_to_sequence(text, dictionary=cmu), len(symbols))).cuda()[
                            None]
                        ...
                        extra_input_dynamic = get_dynamic_extra_input(model_n, extra_input,
                                                                      x, x_lengths, emo_tensor, melstyle_tensor,
                                                                      spk_tensor)
                        generator_input.update(extra_input_dynamic)

                        t = dt.datetime.now()
                        y_enc, y_dec, attn, unet_attn = generator.forward(**generator_input)
                        ...
                        unet_attn = np.array(unet_attn)  # [(t1, [[h, l_q_phoneme_80, l_k_frame_len], [...]]), (t2)]
                        np.save(attn_f, unet_attn)
    ...
    if start_step <= 2 <= end_step:
        ...
        for model_n in ["reference", model_name1, model_name2]:
            ...
            if not os.path.isdir(tg_dir):
                ...
                print("####################")
                print(
                    "Please conduct alignment by 'python mfa_dir.py {} english_mfa {}' under aligner conda enviroment".format(
                        out_sub_dir, tg_dir
                    ))
                print(time.sleep(5))
                print("####################")
            ...
            for speech in speech_list:
                ...
                prosody_dict[emo_id][model_n]["duration"].append(duration.tolist())
    ...
    if start_step <= 3 <= end_step:
        ...
        for vis_speechid in [0, 1]:
            ...
            phonems = prosody_dict["Angry"]["gradtts"]["phonemes"][vis_speechid]
            ...
            print("3.1. do pitch comaration visualization on {}".format(" ".join(
                prosody_dict["Angry"]["gradtts"]["phonemes"][vis_speechid])))
    ...
    if start_step <= 4 <= end_step:
        ...
        for vis_speechid in [0, 1]:
            ...
            ref_phonemes = prosody_dict["Angry"]["gradtts"]["phonemes"][1]
            ...
            for model_n in [model_name1, model_name2]:
                ...
                for attn_map_n in os.listdir(out_attn_sub_dir):
                    ...
                    tgt_ref_f = os.path.join(tgt_ref_dir, id)
                    tgt_syn_f = os.path.join(tgt_syn_dir, id)
        ...
        for model_n, emo_first_last_attn in attn_map_for_vis.items():  # emo_attn: {"emo": (h, p1_len, p2_len)}
            ...
            show_attn_map(
                ...
                out_png=out_png.format("attn_first", model + "_" + "t_{}_h_{}".format(vis_t, vis_h)),
...
if __name__ == "__main__":
    ...
    # extra input (p2p style transfer)
    tgt_pRange = (8, 9)
    ref_pRange = (8, 9)

    # paramaters
    model_name1 = "gradtts"
    model_name2 = "gradtts_cross"
    chk_pt1 = logs_dir + "models/grad_400.pt"
    chk_pt2 = logs_dir + "models/grad_400.pt"
    ...
    configs1 = (preprocess_config, model_config, train_config)
    configs2 = (preprocess_config, model_config, train_config)
    ...
    out_png = "result1/{}_similarity_{}.png"
