...
class CondGradTTS(BaseModule):
    ...
    def forward(self,
        ...
        #spk_emb = self.spk_mlp(self.spk_emb(spk))   ## ?? Remove it ?? seems hard to combine gradTTS/EnhancedTTS in one
        #emo_label_float = emo_label.to(torch.float)
        #emo_emb = self.emo_mlp(emo_label_float)   # (b, 1, 80)
