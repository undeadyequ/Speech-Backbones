...
def hard_monotonic_attention(e_ma, aw_prev, eps_wait, p_threshold=0.5):
    ...
    for q_i in range(qlen):
        _p_choose = torch.sigmoid(e_ma[:, :, q_i:q_i+1])  # for visualization  <- 
        ...
        alphas.append(alpha)
        _p_chooses.append(_p_choose)
        
    alphas = torch.concat(alphas, dim=2)
    _p_chooses = torch.concat(_p_chooses, dim=2)
    return alphas, _p_chooses
