...
from GradTTS.model.utils import (sequence_mask, generate_path, duration_loss,
                                 fix_len_compatibility, align_a2b, align, align_a2b_padcut, cut_pad_start_end, mon_attention_loss, vae_loss,
                                 create_mon_attn_mask)
...
class CondGradTTSDIT(BaseModule):
    ...
    def compute_loss(self,
        ...
        #melstyle_align = align_a2b_padcut(melstyle, y.shape[-1], attn, condtype="seq")   # <- need modification ??
        ...
        # Difussion loss
        melstyle_cond = melstyle_cond.transpose(1, 2)
        ...
        # Monotonic attention loss  <- How to get ground truth loss ??
        ...
        if self.guided_attn:
            ...
            for attn_cross in attn_crosses:
                ...
                monAttn_loss += self.guided_attn(attn_cross, ilens, olens, attn_cross.shape)
            """Ground Truth
        ...
        if self.ref_embed_type == "vae":
            ...
            return dur_loss, prior_loss, diff_loss, monAttn_loss
