...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file', type=str, required=False, default="resources/filelists/synthesis.txt",
                        help='path to a file with texts to synthesize')
    #parser.add_argument('-c', '--checkpoint', type=str, required=False, default="checkpts/grad-tts-libri-tts.pt",
    #                    help='path to a checkpoint of Grad-TTS')
    parser.add_argument('-c', '--checkpoint', type=str, required=False, default="checkpts/grad-tts.pt",
                         help='path to a checkpoint of Grad-TTS')
    ...
    parser.add_argument('-s2', '--speaker_id2', type=int, required=False, default=None,
    ...
    if not isinstance(args.speaker_id2, type(None)):
        assert params.n_spks > 1, "Ensure you set right number of speakers in `params.py`."
        spk2 = torch.LongTensor([args.speaker_id2]).cuda()
    else:
        spk2 = None
    ...
    with torch.no_grad():
        for i, text in enumerate(texts):
            ...
            if spk2 is None:
                ...
                y_enc, y_dec, attn = generator.forward(x, x_lengths, n_timesteps=args.timesteps, temperature=1.5,
                                                       stoc=False, spk=spk, spk2=spk2, length_scale=0.91)
