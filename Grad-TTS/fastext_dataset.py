...
class FastspeechDataset(Dataset):
    def __init__(
        self, filename, preprocess_config, train_config, sort=False, drop_last=False,
            ped=False, iiv=True, s2e=False
    ):
        self.dataset_name = preprocess_config["dataset"]
        self.preprocessed_path = preprocess_config["path"]["preprocessed_path"]
        self.corpus_path = preprocess_config["path"]["corpus_path"]
        self.index_train_f = preprocess_config["path"]["index_train_f"]
    ...
    def __getitem__(self, idx):
        ...
        if self.s2e:
            s2e_path = os.path.join(
                self.corpus_path,
                "emo_reps",
                "{}.npy".format(basename),
            )
            sample["s2e"] = np.load(s2e_path).squeeze(0)
...
class TextFastspeechDataset(FastspeechDataset):
