...
class GradLogPEstimator2dCond(BaseModule):
    def __init__(self, dim, dim_mults=(1, 2, 4), groups=8,
                 n_spks=None, spk_emb_dim=64, n_feats=80, pe_scale=1000, emo_emb_dim=768):
        ...
        dims = [3 + (1 if n_spks > 1 else 0), *map(lambda m: dim * m, dim_mults)]
    ...
    def forward(self, x, mask, mu, t, spk=None, emo=None, style="concatenate"):
        ...
        if not isinstance(spk, type(None)):
            s = self.spk_mlp(spk)
        if not isinstance(emo, type(None)):
            emo = self.emo_mlp(emo)
        ...
        if self.n_spks < 2:
            ...
            if emo is not None:
                if style == "concatenate":
                    emo = emo.unsqueeze(-1).repeat(1, 1, x.shape[-1])  # (batch, emo_emb, mel_emb)
        ...
        if style == "crossattn":
            k, v = torch.stack([mu, x, s], 1)
            q = torch.stack([emo, s], 1)
            x = cross_attn(k, v, q)
        # pass
